...
class DialBertLSTM(object):
    def __init__(
        ...
        batch_size, max_turns, max_seq_length = get_shape_list(
            self.dial_input_ids, expected_rank=3
        )
        ...
        with tf.variable_scope("sent_encoder"):
            sent_model = modeling.BertModel(
                config=self.bert_config,
                is_training=self.is_training,
                input_ids=dial_input_ids,
                input_mask=dial_input_masks,
                token_type_ids=dial_token_types,
                use_one_hot_embeddings=self.use_one_hot_embeddings,
            )
            sent_arrays = sent_model.get_pooled_output()
            hidden_size = sent_arrays.shape[-1]  # .value
            self.sent_arrays = tf.reshape(
                sent_arrays, [batch_size, max_turns, hidden_size]
            )

        with tf.variable_scope("dial_encoder"):
            lstm_cell_fw = tf.compat.v1.nn.rnn_cell.LSTMCell(int(hidden_size / 2))
            lstm_cell_bw = tf.compat.v1.nn.rnn_cell.LSTMCell(int(hidden_size / 2))
            lstm_output, lstm_state = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=lstm_cell_fw,
                cell_bw=lstm_cell_bw,
                inputs=self.sent_arrays,
                sequence_length=true_turns,
                dtype=tf.float32,
            )
...
class DialModel(object):
    ...
    def _create_placeholders(self):
        self.dial_input_ids = tf.placeholder(
            tf.int32, shape=[None, None, None], name="dial_input_ids"
        )
        self.dial_input_masks = tf.placeholder(
            tf.int32, shape=[None, None, None], name="dial_input_masks"
        )
        self.dial_token_types = tf.placeholder(
            tf.int32, shape=[None, None, None], name="dial_token_types"
        )
        self.dial_masks = tf.placeholder(
            tf.int32, shape=[None, None], name="dial_masks"
        )
        self.labels = tf.placeholder(tf.float32, shape=[None, None], name="labels")
        self.target_index = tf.placeholder(
            tf.int32, shape=[None, None], name="target_index"
        )
        self.candi_indexs = tf.placeholder(
            tf.int32, shape=[None, None], name="candi_indexs"
        )
        self.data_cluster = tf.placeholder(
            tf.int32, shape=[None, None], name="data_cluster"
        )
        self.is_training = tf.placeholder(tf.bool, name="is_training")

    def _build_forward(self):
        encode_model = DialBertLSTM(
            self.dial_input_ids,
            self.dial_input_masks,
            self.dial_token_types,
            self.dial_masks,
            self.bert_config,
            self.is_training,
        ...
        dial_arrays = tf.layers.dropout(
            dial_arrays, rate=0.1, training=self.is_training
        )
        ...
        candi_arrays = tf.reshape(
            candi_arrays, [batch_size, self.num_turns, self.hidden_size]
        )
        ...
        fc_b = tf.get_variable(
            "fc_b", [self.hidden_size * 2], initializer=tf.zeros_initializer()
        )
        ...
        with tf.variable_scope("loss"):
            output_layer = tf.layers.dropout(
                mix_arrays, rate=0.1, training=self.is_training
            )
            logits = tf.matmul(output_layer, output_weights, transpose_b=True)
            self.logits = tf.reshape(
                tf.nn.bias_add(logits, output_bias), [batch_size, self.num_turns]
            )
            ...
            cluster_loss = tf.reduce_mean(
                tf.matmul(self.nll, cluster_labels, transpose_b=True)
            ...
            self.loss = (
                (
                    tf.reduce_mean(tf.matmul(self.nll, labels, transpose_b=True))
                    / tf.cast(batch_size, "float32")
                )
                + 0.00 * self.l2_loss
                + cluster_loss
