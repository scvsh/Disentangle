...
class BertConfig(object):
    ...
    def from_json_file(cls, json_file):
        ...
        with tf_io.gfile.GFile(json_file, "r") as reader:
            text = reader.read()
...
class BertModel(object):
    ...
    def __init__(
        ...
        config.hidden_dropout_prob = (
            tf.cast(is_training, tf.float32) * config.hidden_dropout_prob
        )
        config.attention_probs_dropout_prob = (
            tf.cast(is_training, tf.float32) * config.attention_probs_dropout_prob
        )
...
def embedding_postprocessor(
    ...
    if use_position_embeddings:
        ...
        )  # make sure max_position_embeddings larger than seq_lenfth
        with tf.control_dependencies([assert_op]):
            full_position_embeddings = tf.get_variable(
                name=position_embedding_name,
                shape=[max_position_embeddings, width],
                initializer=create_initializer(initializer_range),
            )  # position embedding matrix but not like Attention is all you need
